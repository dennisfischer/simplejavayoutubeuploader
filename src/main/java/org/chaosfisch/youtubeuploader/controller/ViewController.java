...
public class ViewController implements Initializable {
	...
	public static final Template		standardTemplate	= Template.create("embed", true, "mobile", true,
																	"commentvote", true, "rate", true, "comment", 0,
																	"category", 0, "visibility", 0, "videoresponse", 1,
																	"license", 0);
	...
	public void menuAddPlaylist(final ActionEvent event) {
		...
		final Object[] message = { I18nHelper.message("playlistDialog.playlistLabel"), title,
				I18nHelper.message("playlistDialog.descriptionLabel"), summary,
				I18nHelper.message("playlistDialog.playlistPrivate"), playlistPrivate,
				I18nHelper.message("playlistDialog.playlistAccount"), accounts };
		...
		myDialog.setCallback(new EventHandler<ActionEvent>() {
			...
			@Override public void handle(final ActionEvent event) {
				if (!title.getText().isEmpty() && !accounts.getSelectionModel().isEmpty()) {
					playlistService.addYoutubePlaylist((Playlist) Playlist.create("title", title.getText(), "summary",
							summary.getText(), "private", playlistPrivate.isSelected(), "account_id", accounts
									.getValue().getLongId()));
					myDialog.close();
				}
			}
