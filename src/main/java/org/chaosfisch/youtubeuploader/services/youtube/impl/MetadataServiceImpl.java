...
public class MetadataServiceImpl implements MetadataService {
	...
	public String atomBuilder(final Upload queue) {
		...
		videoEntry.mediaGroup.license = (queue.getInteger("license") == 0) ? "youtube"
				: "cc";

		if ((queue.getInteger("visibility") == 2)
				|| (queue.getInteger("visibility") == 3)) {
			videoEntry.mediaGroup.ytPrivate = new Object();
		}

		videoEntry.accessControl.add(new YoutubeAccessControl("embed",
				PermissionStringConverter.convertBoolean(queue
						.getBoolean("embed"))));
		...
		videoEntry.accessControl.add(new YoutubeAccessControl("videoRespond",
				PermissionStringConverter.convertInteger(queue
			...
			public HierarchicalStreamWriter createWriter(final Writer out) {
				return new PrettyPrintWriter(out) {
					...
					public void startNode(final String name,
						...
						isCDATA = name.equals("media:description")
								|| name.equals("media:keywords")
		...
		final String atomData = String.format(
				"<?xml version=\"1.0\" encoding=\"UTF-8\"?>%s",
				xStream.toXML(videoEntry));
	...
	@Override
	public String submitMetadata(final String atomData,
			final File fileToUpload, final Account account)
		...
		final HttpUriRequest request = new Request.Builder(METADATA_UPLOAD_URL,
				...
				.headers(
						ImmutableMap.of("Content-Type",
								"application/atom+xml; charset=UTF-8;", "Slug",
								fileToUpload.getAbsolutePath()))
				.entity(new StringEntity(atomData, Charset.forName("UTF-8")))
				.buildHttpUriRequest();
		// Sign the request
		requestSigner.signWithAuthorization(request,
				authTokenHelper.getAuthHeader(account));
		...
		try {
			...
			if (response.getStatusLine().getStatusCode() == 400) {
				logger.warn("Invalid metadata information: {}; {}",
						response.getStatusLine(),
						EntityUtils.toString(response.getEntity()));
				throw new MetadataException(String.format(
						"Die gegebenen Videoinformationen sind ungültig! %s",
						response.getStatusLine()));
			}
			...
			if (response.getFirstHeader("Location") != null) {
				...
				logger.warn("Metadaten konnten nicht gesendet werden! {}",
						response.getStatusLine());
				throw new MetadataException(
						"Metadaten konnten nicht gesendet werden!");
			}
	...
	private void changeMetadata(final String content) throws IOException,
		...
		try {
			if (upload.getString("thumbnail") != null) {
				...
				thumbnailId = thumbnailService.upload(content,
						upload.getString("thumbnail"),
		...
		final HttpPost postMetaData = new HttpPost(String.format(
		...
		postMetaDataParams
				.add(new BasicNameValuePair("action_edit_video", extractor(
						content, "name=\"action_edit_video\" value=\"", "\"")));

		if (thumbnailId != null) {
			postMetaDataParams.add(new BasicNameValuePair("still_id", "0"));
			postMetaDataParams.add(new BasicNameValuePair(
					"still_id_custom_thumb_version", thumbnailId.toString()));
		} else {
			postMetaDataParams.add(new BasicNameValuePair("still_id", "2"));
			postMetaDataParams.add(new BasicNameValuePair(
					"still_id_custom_thumb_version", ""));
		}

		if (upload.getDate("release") != null) {
			if (upload.getDate("release").after(
		...
		postMetaDataParams.add(new BasicNameValuePair("title", extractor(
				content, "name=\"title\" value=\"", "\"")));
	...
	private String extractor(final String input, final String search,
			final String end) {
		return input.substring(input.indexOf(search) + search.length(),
				input.indexOf(end, input.indexOf(search) + search.length()));
	}

	private String redirectToYoutube(String content) throws IOException,
			ClientProtocolException {
		HttpEntity redirectResponseEntity = null;
		try {
			final HttpUriRequest redirectGet = new HttpGet(extractor(content,
					"location.replace(\"", "\"").replaceAll(
					Pattern.quote("\\x26"), "&").replaceAll(
					Pattern.quote("\\x3d"), "="));

			final HttpResponse redirectResponse = RequestUtil
					.execute(redirectGet);
