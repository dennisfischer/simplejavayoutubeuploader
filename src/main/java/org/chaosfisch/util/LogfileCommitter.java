...
public class LogfileCommitter
{
	...
	public static void commit()
	{
		...
		final HttpUriRequest request = new Request.Builder(COMMIT_URL, Method.POST).headers(ImmutableMap.of("Content-Type",
				...
				.entity(new UrlEncodedFormEntity(logfileParams, Charset.forName("utf-8")))
				.buildHttpUriRequest();
	...
	{
		Setting uuidSetting = Model.findFirst("key = ?", "hidden.uuid");
		if (uuidSetting == null)
		{
			uuidSetting = Model.createIt("key", "hidden.uuid", "value", UUID.randomUUID());
		}
