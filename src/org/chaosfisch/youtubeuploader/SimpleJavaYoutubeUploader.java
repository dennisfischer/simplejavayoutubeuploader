...
public class SimpleJavaYoutubeUploader extends Application
{
	...
	private void initDatabase()
	{
		...
		Base.exec("CREATE TABLE IF NOT EXISTS MESSAGES(ID INTEGER NOT NULL auto_increment PRIMARY KEY,MESSAGE VARCHAR(255),QUEUE_ID INTEGER,FACEBOOK BOOLEAN,TWITTER BOOLEAN,GOOGLEPLUS BOOLEAN,YOUTUBE BOOLEAN, created_at DATETIME, updated_at DATETIME);");
		...
		Base.exec("CREATE TABLE IF NOT EXISTS PLACEHOLDER(ID INTEGER NOT NULL auto_increment PRIMARY KEY,PLACEHOLDER VARCHAR(255),REPLACEMENT VARCHAR(255), created_at DATETIME, updated_at DATETIME);");
		Base.exec("CREATE TABLE IF NOT EXISTS PLAYLISTS(ID INTEGER NOT NULL auto_increment PRIMARY KEY,PKEY VARCHAR(255), PRIVATE BOOLEAN, TITLE VARCHAR(255),URL VARCHAR(255), NUMBER INTEGER, SUMMARY VARCHAR(16777216), ACCOUNT_ID INTEGER, created_at DATETIME, updated_at DATETIME);");
		Base.exec("CREATE TABLE IF NOT EXISTS QUEUE(ID INTEGER NOT NULL auto_increment PRIMARY KEY,ARCHIVED BOOLEAN,CATEGORY VARCHAR(255),COMMENT SMALLINT,COMMENTVOTE BOOLEAN,DESCRIPTION VARCHAR(16777216),EMBED BOOLEAN,FAILED BOOLEAN,FILE VARCHAR(500),VISIBILITY SMALLINT,KEYWORDS VARCHAR(16777216),MIMETYPE VARCHAR(255),MOBILE BOOLEAN,RATE BOOLEAN,TITLE VARCHAR(255),UPLOADURL VARCHAR(255),VIDEORESPONSE SMALLINT,SEQUENCE INTEGER,STARTED TIMESTAMP,INPROGRESS BOOLEAN,LOCKED BOOLEAN,VIDEOID VARCHAR(255),ACCOUNT_ID INTEGER,PLAYLISTS_ID INTEGER,PLAYLIST_ID INTEGER, ENDDIR VARCHAR(255), LICENSE SMALLINT, RELEASE TIMESTAMP,NUMBER SMALLINT, PAUSEONFINISH BOOLEAN, created_at DATETIME, updated_at DATETIME);");
