...
import org.chaosfisch.youtubeuploader.CorePlugin;
import org.chaosfisch.youtubeuploader.services.impl.AccountServiceImpl;
import org.chaosfisch.youtubeuploader.services.impl.PlaceholderServiceImpl;
...
/**
 * Created by IntelliJ IDEA. User: Dennis Date: 26.02.12 Time: 20:17 To change
...
public class MybatisModule extends AbstractModule
{
		...
		install(new CustomMyBatisModule() {
			...
			protected void initialize()
			{
				...
				try
				{
					...
					if (!templateFile.exists()) { throw new RuntimeException("Required file \"mybatis_config_template.xml\" not found."); }
					final String fileContent = MybatisModule.readFileAsString(templateFile);
					...
					try
					{
						...
						for (final Properties prop : appProps)
						{
							...
							{
								if (element.getKey().toString().startsWith("mapper"))
								{ // NON-NLS
	...
	private abstract class CustomMyBatisModule extends AbstractMyBatisModule
	{
		 ...
		 * 
		 * @param properties
		 *            the variables will be used to replace placeholders in the
